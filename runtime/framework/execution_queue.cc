...
absl::Status ExecutionQueue::Cancel(int id) {
  absl::MutexLock lock(mutex_);
  ...
  while (true) {
    absl::AnyInvocable<void() &&> current_task;
    {
      ...
      if (it != pending_tasks_.end()) {
        ...
        // The task was cancelled. Skip it and wait for the next loop.
        continue;
      }
    }

    // Execute the task OUTSIDE the mutex lock.
    // This prevents deadlocks if a task itself calls Enqueue() or Cancel().
    if (current_task) {
